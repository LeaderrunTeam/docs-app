...
| 1.0.7 | 仓转仓`rltInvtNo/turnPutrecNo`设置为非必填项 <br /> 删除 `vehicleList` 字段，<br /> `plate/vehicleWeight`设置为非必填项 | 赖钻   | 2026-06-10 |
| 1.0.8 | 新增补充出库报关单车辆信息接口 | 赖钻   | 2026-06-10 |
| 1.0.9 | 新增`wrapType/otherWrapTypes`字段，非仓转仓业务必填 | 赖钻   | 2026-06-22 |
...
| wrapType | 报关单包装单位       | String(2)            |  非仓转仓业务必填    <Badge text="1.0.9" />                                          |    O     |
| otherWrapTypes | 报关单其他包装单位       | `Set<String>`  | 有多个包装单位在这里补充，需要排除`wrapType`<Badge text="1.0.9" />      |    O     |
| containerStatus    | 柜状态       | Byte                         | 固定填写：10                                           |    Y     |
| trailer            | 是否柜车     | Boolean                      | 柜车填 true，吨车填 false                              |    Y     |
| containerNo        | 柜号         | String(12)                   | 柜车必填                                               |    O     |
| containerWeight    | 柜皮重         | BigDecimal(10,4)             | 柜车必填                                               |    O     |
| containerType      | 柜型         | String(4)                    | 柜车必填                                               |    O     |
| vehicleFrameWeight | 车架重       | BigDecimal(10, 4)            | 柜车必填                                               |    O     |
| vehicleWeight      | 车自重       | BigDecimal(10, 4)            | 车头重量                                                   |    N    |
...
| ~~vehicleList~~   | ~~车辆信息~~           | ~~`List(1...n)`~~    |     <Badge text="1.0.7" />        |    N     |
...
| rltInvtNo    | 关联入仓清单编号   | String(20)   |              |    N     |
...
## 补充车辆信息  <Badge text="1.0.8" />

创建出库订单没有提供车辆资料，可以通过该接口来补充车辆信息

### 请求方法

`POST`：`/v1/outbound/vehicle`
...
```json
{
  "outbdNo": "OE2606100000",
  "vehicleNo": "粤B12345",
  "vehicleType": 0,
  "vehicleWeight": 6500,
  "containerNo": "OOCU8301212",
  "containerType": "40HQ",
  "containerWeight": 3800,
  "vehicleFrameWeight": 2100,
  "driverMobile": "13812345678",
  "driverName": "张三",
  "vehicleFrameNo": "粤BDF610"
}
```
...
| **字段名称**       | **字段描述** | **数据类型** | **详细说明**                                | **必填** |
| ------------------ | ------------ | ------------ | ------------------------------------------- | :------: |
| outbdNo          | 出库订单号   | String(20)    |      创建出仓申报订单得到的单号                                       |    Y     |
| vehicleNo          | 承运车牌号   | String(8)    |                                             |    Y     |
| vehicleType        | 车型         | Byte         | 1 吨车，0 柜车                              |    Y     |
| vehicleWeight      | 车自重（kg） | Short        |  吨车是空车重量，柜车是车头的重量。监管仓业务可以不用填写            |    Y    |
| containerNo        | 集装箱号     | String(11)   | 柜车必填                                    |    O     |
| containerType      | 柜型         | String(4)    | 柜车必填                                    |    O     |
| containerWeight    | 柜皮重         | Short        | 柜车必填，监管仓业务可以不用填写                                 |    O     |
| vehicleFrameWeight | 车架重       | Short        | 柜车必填，监管仓业务可以不用填写                              |    O     |
| driverMobile       | 司机手机号   | String(11)   |                                             |    N     |
| driverName         | 司机姓名     | String(20)   |                                             |    N     |
| vehicleFrameNo     | 车架号       | String(20)   |   柜车必填，吨车默认：1       |    Y     |
| sealNo     | 封条号       | String(20)   |   柜车必填       |    Y     |


### 返回值

修改成功会返回`204`的状态码，没有响应内容
