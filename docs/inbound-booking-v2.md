...
author: 赖钻
sequence: 11
description: 立航接收合作伙伴进仓单数据，支持创建、修改、取消等操作。立航系统会将入仓的各个节点反馈给合作伙伴，包含深圳仓库收货回执、卫星仓收货和出库回执、入仓报关回执。
---
...
| 1.0.4 | 新增 purchaseOrder 字段用来表示采购订单明细  | 赖钻   | 2025-09-08 |
...
| 1.0.9 | 新增`after_updated`仓库回执状态，回执报文内容和`finalized`保持一致  | 赖钻   | 2026-01-05 |
| 1.1.0 | 报关状态`Modify`的时候报文新增返回异常内容节点`causes`  | 赖钻   | 2026-01-27 |
| 1.1.1 | 卫星仓收货完成回执新增[收货核实单](#sat-recAtt)附件返回  | 赖钻   | 2026-03-12 |
| 1.1.2 | 新增`batteryFlag`字段用来表示是否电池货  | 赖钻   | 2026-03-18 |
| 1.1.3 | 卫星仓收货完成新增`唛头相片/收货相片/增值服务打贴标相片`附件返回  | 赖钻   | 2026-04-13 |

## 创建 Booking

创建或修改进仓单数据。相同进仓单号多次请求，立航会默认为修改。进仓单交仓后进仓数据不允许删除，此时立航系统会返回非`200`的状态码
...
| contactPersonEmail | 联系人电子邮件 | String(255)                               | 收货有异常或者系统通知会发送到该邮箱，多个邮箱支持使用英文逗号隔开                                                                                                       |    Y     |
...
| batteryFlag         | 是否电池货   | Boolean                                   | `true`为电池货，空或者`false`非电池货。注意：纯电池货仓仓库代码只能填写`PLA`                                                                                              |    N     |
| sensitive          | 是否敏感货物  <Badge text="1.1.2" /> | Boolean                                   | 如果为`true`必须上传附件，如保函                                                                                                  |    N     |
...
| attachments        | 收货附件     | [List<1..n>](#sat-recAtt) | 状态是`unloading_completed`的时候必填 <Badge text="1.1.1" /> | O |
...
| **字段名称** | **字段描述** | **数据类型**                                                                                                                     | **详细说明** | **必填** |
| ------------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------- | ------------ | :------: |
| docType      | 文件类型     | <Tip text="Enum">marks：唛头相片<br/>cargoReceipt：收货相片<br/>shippingOrder：收货核实单<br/>sticker：增值服务打贴标相片</Tip> |      注意：除了核实单，其它照片不一定有          |    Y     |
    ...
    "attachments": [
      {
        ...
        "fileName": "ZSNLRHAM22121215卫星仓核实单.pdf"
      }
    ]
  }
}
...
| after_updated      <Badge text="1.0.9" />         | 仓库收货数据和工厂提交的报关资料件数或者重量有查差异的情况下，仓库会先做一次复合。如果复合的收货数据有变更会触发此状态 |
...
| **代码**       | **描述**  | **描述** |
| -------------- | ---- | -------- |
| InDelegate     | 委托中 | 工厂提交报关资料   |
| PassAudit      | 审单通过 | 报关资料无异常，到货后即可申报 |
| InDeclaration  | 申报中 | 报关资料已发送海关   |
| EndDeclaration |  申报完毕| SO入仓报关流程结束（最后一个节点） |
| Examine        | 海关查验 |   报关资料申报过程中遇到海关查货   |
| Invalid        | 作废 | SO作废后不可以再提交报关资料     |
| Modify         | 退回修改 |报关资料审核不通过或者工厂要求退单修改 |
...
| causes  | 退回修改原因 | [List](#causes) | 状态为`Modify`才有值   <Badge text="1.1.0" />    |    O     |
...
| **字段名称** | **字段描述** | **数据类型**                                                                    | **详细说明** | **必填** |
| ------------ | ------------ | ------------------------------------------------------------------------------- | ------------ | :------: |
| cause      | 异常原因     | String(200) |      退单原因或者报关资料审核不通过原因        |    Y     |
| name      | 异常类型名称     | String(50) |  |    Y     |
...
```json
{
  ...
  "data": {
    ...
    "attachments": [
      ...
      {
        "docType": "CU000002",
        "fileUrl": "https://uat.leaderrun.com/api/download/cm/customs/2025/4/15/xxx_CU000002.pd",
        "fileName": "xxx_CU000002.pdf"
      },
      {
        "docType": "CU000004",
