...
| 1.0.1 | 新增`shippingOrders`账单关联的进仓单数据字段<br/> 出库账单业务数据新增 `destinationCountry/so`字段 <br/> 新增创建账单返回值说明文档 <br/> 账单明细新增单价和数量字段| 赖钻   | 2026-07-02 |

## 账单推送 {#bill-day-pull}

立航财务对系统生成的日账单审核通过后，系统会自动将账单推送至合作伙伴指定系统。在合作伙伴没有对该账单进行审核之前，立航可能会多次调用接口对账单进行修改。每次修改立航会推送账单的全量数据，包含审核通过以及新增的费用明细数据


立航不会对日账单的明细删除，如需删除费用，立航财务人员会创建一个新增日账单来做红冲
...
| ------------ | ------------ | ------------ | ------------------------------------------------------------ | :------: |
| billNo       | 账单号       | String(20)   | 双方交互的唯一编号，相同单号请求说明是修改操作                     |    Y     |
...
| shippingOrders | 进仓单业务数据 | [List(1...n)](#shippingOrders) | <Badge text="1.0.1" />  | Y |

#### 出库账单业务数据 {#outbd-data}

| **字段名称**  | **字段描述** | **数据类型** | **详细说明**   | **必填** |
| ------------- | ------------ | ------------ | -------------- | :------: |
| containerNo   | 柜号         | String(11)   |       非海运柜不一定有值         |    O     |
| so | 船公司SO   <Badge text="1.0.1" />  | String(32)    | 非海运柜不一定有值 |    O     |
| warehouseCode | 仓库代码     | String(6)    | [深圳仓仓库代码](./basic-data#warehouseCode) |    Y     |
| destinationCountry  | 目的国  <Badge text="1.0.1" />   | String(3)    | [国家代码表](./basic-data/country)   |    Y     |
...
| quantity | 数量 | Float(10,2) | <Badge text="1.0.1" />  | Y |
| price | 单价 | Float(10,2) | <Badge text="1.0.1" />  合同单价 | Y |
...
#### 进仓单业务数据 {#shippingOrders} <Badge text="1.0.1" />

| **字段名称** | **字段描述** | **数据类型** | **详细说明**                                     | **必填** |
| ------------ | ------------ | ------------ | ------------------------------------------------ | :------: |
| bookingNo  | 进仓单号     | String(32)    |  |    Y     |
| destinationCountry  | 目的国     | String(3)    | [国家代码表](./basic-data/country)   |    Y     |
| satWhseCode  | 网点代码     | String(32)    | [卫星仓仓库代码](./basic-data#sateWarehouseCode) |    N    |

#### 返回值 <Badge text="1.0.1" />

:::warning 提示

处理成功需要返回Http Status Code 200的状态码，200状态可以不用返回payload data

处理失败需要返回Http Status Code 400状态码，需要返回下方表格定义的payload data

:::

| **字段名称** | **字段描述** | **数据类型** | **详细说明**                                     | **必填** |
| ------------ | ------------ | ------------ | ------------------------------------------------ | :------: |
| status  |    数据状态  |   Integer  |  |    Y     |
| message  | 状态描述     | String(200)    |   |    Y     |
...
| 200  |    处理成功  |  
| 100  | 需要转线下人工处理     | 
| 999  | 上述代码未包含的错误，该状态下立航会进行重试。[查看重试机制](./guide#retry-rule)    | 
...
| **字段名称** | **字段描述**   | **数据类型**                                                 | **详细说明**                             | **必填** |
| ------------ | -------------- | ------------------------------------------------------------ | ---------------------------------------- | :------: |
| billNo       | 账单号         | String(20)                                                   | 账单推送内容中的账单号 |    Y     |
